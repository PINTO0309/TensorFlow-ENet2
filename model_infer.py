...
from utilities import PReLU, max_unpool

class ENet_model(object):

    def __init__(self, img_height=512, img_width=1024, batch_size=4):
    ...
    def add_placeholders(self):
        self.imgs_ph = tf.placeholder(tf.float32, shape=[self.batch_size, self.img_height, self.img_width, 3], name="imgs_ph")
    ...
    def add_logits(self):
        ...
        # decoder:
        # # layer 4:
        network = self.decoder_bottleneck(x=network, output_depth=64, scope="bottleneck_4_0", upsampling=True, pooling_indices=pooling_indices_2, output_shape=inputs_shape_2)

        network = self.decoder_bottleneck(x=network, output_depth=64, scope="bottleneck_4_1")

        network = self.decoder_bottleneck(x=network, output_depth=64, scope="bottleneck_4_2")


        # # layer 5:
        network = self.decoder_bottleneck(x=network, output_depth=16, scope="bottleneck_5_0", upsampling=True, pooling_indices=pooling_indices_1, output_shape=inputs_shape_1)

        network = self.decoder_bottleneck(x=network, output_depth=16, scope="bottleneck_5_1")



        # fullconv:
        network = tf.contrib.slim.conv2d_transpose(network, self.no_of_classes, [2, 2], stride=2, scope="fullconv", padding="SAME")
    ...
    def encoder_bottleneck_regular(self, x, output_depth, scope, proj_ratio=4, downsampling=False):
        ...
        if downsampling:
            W_conv = self.get_variable_weight_decay(scope + "/W_proj",
                        ...
                        initializer=tf.contrib.layers.xavier_initializer(),
                        loss_category="encoder_wd_losses")
            conv_branch = tf.nn.conv2d(conv_branch, W_conv, strides=[1, 2, 2, 1], padding="VALID") # NOTE! no bias terms
        else:
        ...
        if downsampling:
            ...
            main_branch, pooling_indices = tf.nn.max_pool_with_argmax(main_branch, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME")
